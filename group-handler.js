      ...
      for (const jid of participants) {
         ...
         if (action == "add" && config.WELCOME ) {
           const userName = jid.split("@")[0];
                    const joinTime = moment.tz('Africa/Zimbabwe').format('HH:mm:ss');
                    const joinDate = moment.tz('Africa/Zimbabwe').format('DD/MM/YYYY');
...
"`, contextInfo: {
           ...
           const userName = jid.split('@')[0];
                    const leaveTime = moment.tz('Africa/Zimbabwe').format('HH:mm:ss');
                    const leaveDate = moment.tz('Africa/Zimbabwe').format('DD/MM/YYYY');
                    const membersCount = metadata.participants.length;
            sock.sendMessage(id, {
